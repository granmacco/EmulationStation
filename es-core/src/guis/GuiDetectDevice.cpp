	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BIENVENIDO" : "CONFIGURAR CONTROLES (MAPEADO)",
	...
	if(numDevices > 0)
		deviceInfo << numDevices << " CONTROLADOR DE JUEGO" << (numDevices > 1 ? "S" : "") << " DETECTADO";
	else
		deviceInfo << "NO SE HAN DETECTADO CONTROLADORES DE JUEGOS";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "MANTENGA PRESIONADO UN BOTÓN DEL DISPOSITIVO A CONFIGURAR.", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PRESIONE F4 PARA CERRAR EMULATIONSTATION." : "PULSE ESC PARA CANCELAR.";
