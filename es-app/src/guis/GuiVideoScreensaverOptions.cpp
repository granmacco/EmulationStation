...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("CAMBIAR VÍDEO DESPUÉS DE (SEGS)", swap);
	addSaveFunc([swap] {
	...
	addWithLabel("USAR REPRODUCTOR OMX COMO SALVAPANTALLAS", ss_omx);
	...
	auto ss_vlc_resolution = std::make_shared< OptionListComponent<std::string> >(mWindow, "ALINEACIÓN DE INFORMACIÓN", false);
	...
	addWithLabel("VLC: RESOLUCIÓN DEL VIDEO SALVAPANTALLAS", ss_vlc_resolution);
	...
	addWithLabel("OMX: ALINEACIÓN DE INFORMACIÓN", ss_omx_subs_align);
	...
	addWithLabel("OMX: TAMAÑO DE LA LETRA DE INFORMACIÓN", ss_omx_font_size);
	...
	addEditableTextComponent(row, "OMX: RUTA DEL TIPO DE LETRA CURSIVA", ss_omx_italic_font_file, Settings::getInstance()->getString("SubtitleItalicFont"));
