	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "SCRAPEADO EN PROGRESO", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	mSubtitle = std::make_shared<TextComponent>(mWindow, "texto de subtitulo", Font::get(FONT_SIZE_SMALL), 0x888888FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		...
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SALTAR", "saltar", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "DETENER", "detener (progreso guardado)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NO SE SCRAPEARON LOS JUEGOS.";
	}else{
		ss << mTotalSuccessful << " JUEGO" << ((mTotalSuccessful > 1) ? "S" : "") << " SCRAPEADO CON ÉXITO!";
