...
void GuiMenu::openUISettings()
{
	...
	s->addWithLabel("IGNORAR ARTICULOS (SÓLO NOMBRES CORTOS)", ignore_articles);
	s->addSaveFunc([ignore_articles, window] {
		...
		if (ignore_articles->getState() != articles_are_ignored)
		{
			...
			GuiInfoPopup* popup = new GuiInfoPopup(window, "Ficheros ordenados", 4000);
...
void GuiMenu::openOtherSettings()
{
	...
	s->addWithLabel("ÚNICAMENTE ANALIZAR LISTA DE JUEGOS", parse_gamelists);
	...
	s->addWithLabel("USAR OMX PLAYER (ACELERADO POR HW)", omx_player);
